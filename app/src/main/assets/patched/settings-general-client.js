	...
	factory: (require) => {
		...
		*/
		function GeneralSection({ renderSlot }) {
			...
			const [zoom, setZoom] = (0, react.useState)(() => {
				try {
					return parseInt(localStorage.getItem("dsh.android.textZoom") || "", 10) || 100;
				} catch (e) {
					return 100;
				}
			});
			(0, react.useEffect)(() => {
				try {
					localStorage.setItem("dsh.android.textZoom", String(zoom));
					if (window.androidBridge && typeof window.androidBridge.setTextZoom === "function") {
						window.androidBridge.setTextZoom(zoom);
					}
				} catch (e) {}
			}, [zoom]);
			/* dsh-mobile-textzoom-sync: push the persisted value through the
			 * native bridge once on mount, so SharedPreferences and the WebView
			 * settle even when the index.html restore script was cache-missed. */
			(0, react.useEffect)(() => {
				try {
					if (window.androidBridge && typeof window.androidBridge.setTextZoom === "function") {
						window.androidBridge.setTextZoom(zoom);
			...
			const rowStyle = { display: "flex", alignItems: "center", gap: "12px", padding: "10px 0", borderBottom: "1px solid var(--dsw-alias-border-l1)" };
			const labelStyle = { color: "var(--dsw-alias-label-primary)", fontSize: "14px", lineHeight: "20px", flex: "1" };
			const sliderStyle = { flex: "none", width: "160px", accentColor: "var(--dsw-alias-accent)" };
			const valueStyle = { color: "var(--dsw-alias-label-secondary)", fontSize: "13px", lineHeight: "20px", width: "44px", textAlign: "right", fontVariantNumeric: "tabular-nums" };
