...
| **OverlayService.kt** / **OverlayController.kt** | 悬浮球 v2（PRD-overlay-v2 rev5，0.13.2-fix 从零重写）：**收起=纯黑白球**（ic_launcher_foreground bbox 裁剪居中，状态=低饱和蓝/红光环绕，球体不随状态变色）+ **展开=上下合体圆角矩形**（radius 30dp：会话选择器行 + 状态行 deep diving 扫光 ShimmerTextView + 工具×N 徽标 + 时钟 / 输入行 EditText+蓝底白箭发送+红底白框停止 rx=3） | 双维状态解耦（引擎=EngineProbe 探活 / 会话=live 流 tool_call 计数，**busy 会话感知**——仅当前目标会话的 tool_call/turn_end 驱动）；发送=session.prompt steer（插话）/queue，**目标会话=展开态下拉选择器**（session.list 投影，空目标自动 session.create）；停止=session.cancel 仅工作中可用；**IME 展开 SOFT_INPUT_ADJUST_NOTHING**（禁系统 pan 抬高窗口——回归修复）；spring 吸附（M3 Expressive 380/0.8）；引擎页避让帧保留 |
